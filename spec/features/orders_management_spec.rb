...
RSpec.feature "Orders Management", type: :feature do
  ...
  let!(:users) { create_list(:user, 2) } # Create multiple users for testing
  let!(:order) { create(:order, user: users.first) }
  let!(:product) { create(:product) } # Optional: Add products if needed for testing
  ...
  scenario "Admin views list of orders" do
    ...
    expect(page).to have_content(order.user.email) # Ensure the correct user email is displayed
  end
